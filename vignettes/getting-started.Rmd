...
**`su1_contact_mat`** is a 649×649 numeric contact matrix representing human chromosome 21,
...
- *MCMC progress*: dimensions confirmed, pre-processing steps (clustering, backbone),
  initialization, and per-iteration log-likelihood trace
  (e.g. `Iter 1000/1000. LL: 166134. Best LL: 166134`).
- *Acceptance rates* for β0, β1, center positions, and locus positions (~22–25% is healthy).
- *Summary of files* saved to `output_dir`: `final_positions.txt`, `initial_positions.txt`,
  `log_likelihood_trace.txt`, `block_timings.txt`, `mcmc_log.txt`.
